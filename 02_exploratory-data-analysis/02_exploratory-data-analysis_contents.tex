...
\begin{frame}{Variable types decide the analysis}
    ...
    \begin{itemize}
        ...
        \item The type decides which summary is meaningful: gender could be stored as 1, 2, yet an `average gender' of 1.5 is meaningless
    \end{itemize}
\end{frame}
...
\begin{frame}{What is EDA?}
    \begin{sitemize}
        \item Exploratory data analysis (EDA) is the systematic examination of a dataset through summary statistics and graphics before any formal modelling.
...
\begin{frame}{Why look at the data? An example}
    \textbf{The Diamonds dataset:} 53,940 stones, price in US dollars. Mean price by cut, from worst to best:
    \begin{center}
    \footnotesize
    \begin{tabular}{lrrr}
        \toprule
    ...
    \footnotesize
    \begin{itemize}
        \item The best cut (Ideal) has the lowest mean price, and the worst cut (Fair) is more expensive than Good, Very Good, and Ideal
...
\begin{frame}{First look at a dataset}
    \begin{itemize}
        ...
        \item Duplicate rows and impossible values, such as negative ages or zero height/weight
        % \item Titanic (891 passengers): age is missing for 177 passengers, deck for 688, and port of embarkation for 2
    \end{itemize}
\end{frame}


\section{Univariate Analysis: One Variable at a Time}
...
\begin{frame}{Boxplot}
    ...
    \footnotesize
    \begin{itemize}
        \item \textbf{Boxplot:} box from $Q_1$ to $Q_3$ with the median line; whiskers reach the furthest points within $1.5\times\mathrm{IQR}$; points beyond are flagged
        \begin{itemize}
            \item Use to compare many groups side by side
            \item It cannot show several peaks
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Two numeric variables: scatter plots}
    ...
    \footnotesize
    \begin{itemize}
        \item Above is a scatterplot that shows the relationship between car weight and car mileage using the `mtcars' dataset
        \item The overall shape of the plot can tell us whether there is any association between the two variables
        \item The correlation coefficient is a numeric measure that quantifies the linear relationship between the two variables
    \end{itemize}
\end{frame}
...
\begin{frame}{Numeric variable across groups}
    ...
    \footnotesize
    \begin{itemize}
        \item Above are boxplots of body mass of different species of penguins
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.50\textwidth}
            ...
            \begin{itemize}
                ...
                \item The correlation heatmap shows only the pairwise linear association among the numeric variables of a dataset
                \item The current plot uses the `mtcars' dataset
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{More dimensions: colour, shape, facets, parallel coordinates}
    ...
    \begin{itemize}
        ...
        \item Facets by island reveal that Gentoo lives only on Biscoe and Chinstrap only on Dream, while Adelie is on all three
    \end{itemize}
\end{frame}
...
\begin{frame}{Time series: trend, seasonality, variance}
    \begin{center}
        \includegraphics[width=0.95\linewidth,height=4.5cm,keepaspectratio]{figures/eda_airpassengers.pdf}
    \end{center}
    \footnotesize
    \begin{itemize}
        \item Plot against time first; look for trend, seasonality, changing variance, and breaks
        \item Above is a time series and a seasonality plot of the `air passengers' dataset
        \item The yearly mean rises from 126.7 to 476.2 thousand over 1949--1960
        \item Seasonal peak in July and August
    \end{itemize}
\end{frame}


\begin{frame}{Spatial data: map before you model}
    \begin{center}
        \includegraphics[width=0.95\linewidth,height=3.2cm,keepaspectratio]{figures/eda_california_maps.pdf}
    \end{center}
    \footnotesize
    \begin{itemize}
        \item Map each variable (20,640 districts): values are high near the coast, low inland (median \$108,500 inland vs. \$214,850--\$233,800 coastal)
        \item Observations are correlated, nearby observations resemble each other
        \item Observations are not independent, so random train/test splits overstate accuracy
        \item Check coordinates and resolution: survey cluster locations (e.g. DHS) are deliberately displaced
    \end{itemize}
\end{frame}
...
\begin{frame}{EDA checklist and tools}
    \begin{enumerate}
        \item First look: data size, types, missing, duplicates
        \item Each variable: summaries, histogram or bar chart
        \item Quality: outliers vs. artefacts, caps, codes
        \item Pairs: plots, correlations, groups, confounders
        \item Structure: time order, space, groups
    \end{enumerate}
\end{frame}
